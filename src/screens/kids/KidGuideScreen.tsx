...
export function KidGuideScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [list, config] = await Promise.all([guideContentService.listNights(), getHanukkahConfig()]);
    setNights(list);
    const status = getHanukkahStatus(config.startsOn, new Date());
    setExpandedNight(status.phase === 'during' && status.night ? status.night : 1);
    setLoading(false);
  }, []);
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: colors.bgPrimary }]} edges={['top']}>
      <ScrollView style={styles.flex} contentContainerStyle={[styles.content, webFrame]}>
        <TouchableOpacity
