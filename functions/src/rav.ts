...
type RavResponse = { text: string; blocks: RavBlock[]; actions?: RavDraftAction[] };
...
async function buildHouseholdContext(uid: string, clientDraft?: string): Promise<string> {
  const db = getFirestore();
...
export const askPilotRav = onCall(
  ...
  async (request): Promise<{ reply: string; text: string; blocks: RavBlock[]; actions: RavDraftAction[] }> => {
